...
def parse_args():
	...
	parser.add_argument('path', help = '<input> File name you would like to merge.')
	parser.add_argument('additional usage', help = '<example> python3 Caron_Final_Project.py <filename.csv>')
...
def file_choices(choices,fname): 
	...
	if ext not in choices:
		parser.error('File does not end with extension {}'.format(choices)) # choices are listed in main body of code below (can be added to later, if needed)
	return fname
...
# if not enough/too many arguments entered into command line, print helpful message to command line, includes usage prompt
	print('Incorrect number of arguments. For usage information, enter: <python3> <Caron_Final_Project.py> <-h>')


parse_args()
parser.add_argument('fn', type = lambda s:file_choices(('csv'),s)) # file extensions that will allow code to run, see function file_choices
parser.parse_args()
